--[[
  ...
  * license MIT
--]] -- Variables --
local MOVE_UP_KEY = 0xDE794E3E
local MOVE_DOWN_KEY = 0xCEFD9220
...
local noClippingEntity = cache.ped;
local shownControls = false
...
local function SetNoClip(val)
    if (isNoClipping ~= val) then
        ...
        if (isNoClipping) then
            if not shownControls then
                lib.showTextUI(('%s  \n%s  \n%s  \n%s'):format(locale('noclip_controls'), locale('noclip_fw_bw'), locale('noclip_left_right'), locale('noclip_up_down')), { position = "left-center", })
                shownControls = true
            end
            ...
            CreateThread(function()
                ...
                while isNoClipping do
                    ...
                    input = vector3(GetControlNormal(0, MOVE_LEFT_RIGHT), GetControlNormal(0, MOVE_UP_DOWN), (IsControlAlwaysPressed(1, MOVE_DOWN_KEY) and 1) or ((IsControlAlwaysPressed(1, MOVE_UP_KEY) and -1) or 0))
...
AddEventHandler('onResourceStop', function(resourceName)
    if resourceName ~= RESSOURCE_NAME then return end
    SetNoClip(false);
    ...
    ResetEntityAlpha(noClippingEntity);
    SetEveryoneIgnorePlayer(noClippingEntity, false);
    ResetEntityAlpha(noClippingEntity);
